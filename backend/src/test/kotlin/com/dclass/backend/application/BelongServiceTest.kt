...
class BelongServiceTest : BehaviorSpec(
    {
        ...
        Given("두 개의 학과가 존재하는 경우") {
            ...
            every { departmentRepository.getByTitleOrThrow(request.major) } returns department(
                id = 3L,
                title = request.major,
            )
            every { departmentRepository.getByTitleOrThrow(request.minor) } returns department(
                id = 4L,
                title = request.minor,
            )
